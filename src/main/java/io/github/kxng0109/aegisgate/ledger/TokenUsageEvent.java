...
/**
 * Carries the usage and cost of one completed streaming request from the hot
 * path to the asynchronous ledger.
 *
 * <p>The event is published exactly once, after the stream finished, and is
 * handled on a dedicated executor so the publishing thread never blocks. It
 * carries no secrets: the owner id is the tenant identifier resolved from the
 * virtual API key, and the provider is the winning provider name.</p>
 *
