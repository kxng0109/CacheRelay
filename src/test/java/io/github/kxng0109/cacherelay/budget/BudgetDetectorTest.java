...
class BudgetDetectorTest {
	...
	void slowBurnFiresWarning() {
		...
		try (MockedStatic<BudgetEnforcer> mocked = mockStatic(BudgetEnforcer.class, CALLS_REAL_METHODS)) {
			mocked.when(BudgetEnforcer::secondsToMonthEnd).thenReturn(1_800_000L);
			for (int i = 0; i < 85; i++) {
				harness.detector().evaluateAll(tick.plusSeconds(60L * i));
			}
			List<AlertEvent> rows = alertsOf(savedAlerts(harness.outbox()), "burn_forecast");
			assertThat(rows).hasSize(1);
			assertThat(rows.getFirst().getSeverity()).isEqualTo("warning");
		}
	}
