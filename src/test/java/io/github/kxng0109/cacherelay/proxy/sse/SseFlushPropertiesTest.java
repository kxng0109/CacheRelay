 ...
 * {@code cacherelay.sse.flush} prefix, and the {@code @Min}/{@code @Max} constraints.
 */
...
class SseFlushPropertiesTest {
	...
	@Test
	@DisplayName("properties bind from the cacherelay.sse.flush prefix with relaxed naming")
	void bindsFromPrefix() {
		StandardEnvironment environment = new StandardEnvironment();
		environment.getPropertySources().addFirst(new MockPropertySource("test")
				                                          .withProperty("cacherelay.sse.flush.max-lines-per-flush", "32")
				                                          .withProperty("cacherelay.sse.flush.max-interval-ms", "250")
				                                          .withProperty("cacherelay.sse.flush.enabled", "false"));

		SseFlushProperties bound = Binder.get(environment)
		                                 .bind("cacherelay.sse.flush", SseFlushProperties.class)
		                                 .get();
	...
	void partialPropertiesUseDefaults() {
		...
		environment.getPropertySources().addFirst(new MockPropertySource("test")
				                                          .withProperty(
						                                          "cacherelay.sse.flush.max-lines-per-flush",
						                                          "48"
				                                          ));
