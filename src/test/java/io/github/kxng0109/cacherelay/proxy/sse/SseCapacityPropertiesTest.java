...
class SseCapacityPropertiesTest {
	...
	@Test
	@DisplayName("properties bind from the cacherelay.sse.capacity prefix with relaxed naming")
	void bindsFromPrefix() {
		StandardEnvironment environment = new StandardEnvironment();
		environment.getPropertySources().addFirst(new MockPropertySource("test")
				                                          .withProperty(
						                                          "cacherelay.sse.capacity.max-connections",
						                                          "60000"
				                                          )
				                                          .withProperty("cacherelay.sse.capacity.tick-period-ms", "5")
				                                          .withProperty(
						                                          "cacherelay.sse.capacity.watchdog-timeout-ms",
						                                          "15000"
				                                          ));

		SseCapacityProperties bound = Binder.get(environment)
		                                    .bind("cacherelay.sse.capacity", SseCapacityProperties.class)
		                                    .get();
