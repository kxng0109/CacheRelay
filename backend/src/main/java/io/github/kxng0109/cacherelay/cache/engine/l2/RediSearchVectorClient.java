...
public class RediSearchVectorClient {
	 ...
	 *
	 * @param indexName  index identifier (e.g. {@code cacherelay:cache:idx})
	 * @param prefix     key prefix to index (e.g. {@code cacherelay:cache:doc:})
	...
	private double parseScore(Object scoreObj, String docKey) {
		...
		try {
			...
			try {
				Counter.builder("cacherelay.cache.vector.search.nan_scores")
				       .tag("key", docKey)
	...
	private void recordFailure(String causeClass) {
		try {
			Counter.builder("cacherelay.cache.vector.search.failures")
	...
	private void recordRetry(String causeClass) {
		try {
			Counter.builder("cacherelay.cache.vector.search.retries")
			       .tag("cause", causeClass)
			       .register(meterRegistry)
			       .increment();
		} catch (Exception ignored) {
	 ...
	 *
	 * @param docKey document key (e.g. {@code cacherelay:cache:doc:tenant:id})
	...
	public void saveVectorDocument(String docKey, Map<byte[], byte[]> fields, Duration ttl) {
		...
		try (RedisConnection connection = redisConnectionFactory.getConnection()) {
			if (isZeroNormVector(fields)) {
				try {
					Counter.builder("cacherelay.cache.vector.store.zero_norm_rejected")
					       .register(meterRegistry)
					       .increment();
				} catch (Exception ignored) {
				}
