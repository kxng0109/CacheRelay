...
/**
 * Generic HTTPS webhooks with Stripe-style HMAC-SHA256 signing: {@code X-CacheRelay-Timestamp} (unix seconds) and
 * {@code X-CacheRelay-Signature: t=&lt;ts&gt;,v1=&lt;hex(hmac(secret, ts + "." + body))&gt;}. Receivers reject
...
public class WebhookSender extends BaseSender implements ChannelSender {
	...
	public ChannelResult send(NotificationPreference preference, NotificationPayload payload) {
		...
		return postRaw(preference.getTarget(), body, Map.of(
				"X-CacheRelay-Timestamp", timestamp,
				"X-CacheRelay-Signature", signature));
	}
}
