...
class RateLimitEngineTest {
	...
	void decisionCounterRecordsAllowedAndDenied() {
		...
		assertEquals(1.0, registry.get("cacherelay.ratelimit.evaluations").tag("decision", "allowed").counter().count());
		assertEquals(1.0, registry.get("cacherelay.ratelimit.evaluations").tag("decision", "denied").counter().count());
	}
