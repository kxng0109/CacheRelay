...
class OpenApiConfigTest {
	...
	@Test
	@DisplayName("cacheRelayOpenAPI registers complete OpenAPI 3.1 metadata and security components")
	void cacheRelayOpenAPI() {
		OpenAPI openAPI = openApiConfig.cacheRelayOpenAPI();

		assertThat(openAPI.getOpenapi()).isEqualTo("3.1.0");
		assertThat(openAPI.getInfo()).isNotNull();
		assertThat(openAPI.getInfo().getTitle()).contains("CacheRelay");
		assertThat(openAPI.getInfo().getVersion()).isEqualTo("1.7.0");
