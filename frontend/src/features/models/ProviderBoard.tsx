...
export function validationBadge(status: string): { classes: string; label: string } {
  switch (status) {
    ...
    case 'AUTH_REACHABLE':
      // No blue token exists by design (one accent + status colors only),
      // so reachability reads as the warn pair: pending, not failed.
      return { classes: 'bg-warn/15 text-warn-deep dark:text-warn-soft', label: '● Reachable' }
    case 'LIVE_VERIFIED':
      return {
        classes: 'bg-success/15 text-success-deep dark:text-success-soft',
        label: '● Live',
      }
    case 'UNVERIFIED':
      return {
        classes: 'bg-danger/15 text-danger-deep dark:text-danger-soft',
        label: '■ Unverified',
      }
    default:
      return {
